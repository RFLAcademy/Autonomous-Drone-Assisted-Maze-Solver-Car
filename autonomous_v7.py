# integrated_waypoint_nav_v3.py
# Uses your EXACT path detection algorithm + your step-control driver
# Strategy: start -> next turning point -> ... -> end, then stop.
# Update: forward motion uses continuous boost/cruise (90 → ≥65), turns remain step-based.

# v6 is same as v7 but with p controller applied. delay means it was not effective.
# v7 has longer straight sections instead of stop/start
import cv2
import numpy as np
...
YAW_TOLERANCE = 10
FORWARD_POWER = 90      # requested forward power (ESP32 clamps to ≥65 after pickup=90)
TURN_POWER = 90
STEP_DURATION = 0.15
SETTLE_DELAY = 0.5
...
def main():
    ...
    while True:
        ...
        if start_px is not None and end_px is not None:
            ...
            if start_px is None: print("⚠️ AprilTag start not detected")
            if end_px is None: print("⚠️ Blue end not detected")
    ...
    if path_pixels:
        cv2.circle(output, path_pixels[0], 5, (0, 255, 255), -1)
        cv2.circle(output, path_pixels[-1], 5, (255, 0, 255), -1)
    ...
    while current_idx < total_targets:
        ...
        if dist < arrive_r:
            if is_moving_forward:
                try:
                    requests.get(f"{ESP32_IP}/stop", timeout=0.5)
                    print("⏹ Stop at waypoint")
                except:
                    print("⚠️ Stop failed at waypoint")
                is_moving_forward = False
        ...
        if abs(diff) > YAW_TOLERANCE:
            if is_moving_forward:
                try:
                    ...
                    print("⚠️ Stop failed before turn")
                is_moving_forward = False
            if diff > 0:
            ...
            if not is_moving_forward:
                try:
                    url = f"{ESP32_IP}/forward?leftPower={FORWARD_POWER}&rightPower={FORWARD_POWER}"
                    requests.get(url, timeout=0.5)
                    print(f"▶️ Continuous forward: {url}")
